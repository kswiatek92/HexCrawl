"""Integration tests for ``POST /v1/game/{id}/abandon`` (task 3.8).

The route is exercised through ``TestClient`` with the ``AbandonGame`` use case
wired to hand-written port fakes (no DB / Redis) and the auth dependency
overridden — the same harness style as ``test_game_get.py``. The real use case
and the real schema mapping run; only the persistence ports are faked.

Auth is overridden two ways depending on the test: most tests override
``get_current_user`` with a fixed principal; the "auth required" test instead
stubs ``get_verifier`` and sends no token so the real dependency returns 401.
"""
...
from src.entrypoints.http.dependencies import get_abandon_game
...
def _make_app(
    ...
    *,
    ...
    """Build an app whose ``AbandonGame`` uses ``repo``/``cache``.
    ...
    app.dependency_overrides[get_abandon_game] = lambda: AbandonGame(repo, cache)
...
def test_abandon_returns_200_with_final_state_for_owner(
    ...
    resp = client.post(f"/v1/game/{game_id}/abandon")

    assert resp.status_code == 200
    body = resp.json()
    assert UUID(body["game_id"]) == game_id
    assert body["seed"] == 42
    # The Abandon action ran a turn, so the final state reflects turn_count + 1.
...
def test_abandon_unknown_id_is_404(repo: FakeGameRepository, cache: FakeCachePort) -> None:
    app = _make_app(repo, cache, principal=AuthenticatedUser(user_id=uuid4()))
    client = TestClient(app)

    resp = client.post(f"/v1/game/{uuid4()}/abandon")
...
def test_abandon_foreign_run_is_403(repo: FakeGameRepository, cache: FakeCachePort) -> None:
    owner = uuid4()
    intruder = uuid4()
    ...
    # A rejected abandon must not have ended the run (no cache write happened).
    assert cache.store == {}
...
def test_abandon_unauthenticated_is_401(repo: FakeGameRepository, cache: FakeCachePort) -> None:
...
def test_abandon_non_uuid_id_is_422(repo: FakeGameRepository, cache: FakeCachePort) -> None:
    app = _make_app(repo, cache, principal=AuthenticatedUser(user_id=uuid4()))
    client = TestClient(app)

    resp = client.post("/v1/game/not-a-uuid/abandon")
