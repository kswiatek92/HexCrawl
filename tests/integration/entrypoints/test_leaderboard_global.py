"""Integration tests for ``GET /v1/leaderboard/global`` (task 3.10).
...
``test_game_abandon.py``. The real use case, schema mapping, and query-param
validation run; only the persistence ports are faked.

The global board is **public**, so — unlike the ``/game/*`` tests — no auth
dependency is overridden and requests carry no token. One test asserts that
explicitly (no token still yields 200), distinguishing this endpoint from the
authenticated ``/leaderboard/me`` (task 3.12).
"""
...
def _make_app(repo: FakeScoreRepository, cache: FakeCachePort) -> FastAPI:
    ...
    No auth override: the global board is public, so the real (absent) auth path
...
def test_returns_ranked_entries_on_cache_miss() -> None:
    ...
    body = resp.json()
    assert body["period"] == "GLOBAL"
    assert [e["rank"] for e in body["entries"]] == [1, 2]
    assert body["entries"][0]["user_id"] == str(top_user)
    assert body["entries"][0]["value"] == 900
    # Miss path consulted the repo and back-filled the cache.
    assert repo.top_n_calls == [(100, LeaderboardPeriod.GLOBAL)]
...
def test_pagination_slices_and_keeps_absolute_ranks() -> None:
    ...
    resp = client.get("/v1/leaderboard/global?limit=2&offset=1")
...
def test_offset_past_end_returns_empty_page() -> None:
    ...
    resp = client.get("/v1/leaderboard/global?offset=50")
...
def test_limit_over_cap_is_rejected() -> None:
    ...
    assert client.get("/v1/leaderboard/global?limit=101").status_code == 422
    assert client.get("/v1/leaderboard/global?limit=0").status_code == 422


def test_negative_offset_is_rejected() -> None:
    client = TestClient(_make_app(FakeScoreRepository([]), FakeCachePort()))

    assert client.get("/v1/leaderboard/global?offset=-1").status_code == 422
