...
### Score (task 1.7)
- [x] Exact formula — literally `floors × kills × item_multiplier`, or weighted (e.g. `floors² × kills × item_multiplier`)? → **Weighted on floors.** Depth dominates so descending is strictly better than grinding shallow floors. Exact exponent (likely `floors²`) to be pinned during task 1.7 / 1.18 implementation.
